...
public class HipertensoDAOImpl implements HipertensoDAO{
	...
	public void salvar(Hipertenso hipertenso) {
		try {
			...
			System.out.println("Não foi possível inserir o hipertenso. Erro: " + e.getMessage());
		} finally {
			try {
	...
	public void atualizar(Hipertenso hipertenso) {
		...
		try {
			...
			System.out.println("Não foi possível alterar o hipertenso. Erro: " + e.getMessage());
		} finally {
			try {
				if (this.sessao.isOpen())
					sessao.close();
	...
	public void excluir(Hipertenso hipertenso) {
		...
		try {
			...
			System.out.println("Não foi possível excluir o hipertenso. Erro: " + e.getMessage());
		} finally {
			try {
				sessao.close();
			} catch (Throwable e2) {
