...
public class AgenteDAOImpl implements AgenteDAO{
	...
	public void salvar(Agente agente) {
		try {
			...
			System.out.println("Não foi possivel inserir o Agente. Erro: " + e.getMessage());
		} finally {
			try {
	...
	public void atualizar(Agente agente) {
		...
		try {
			...
			System.out.println("Não foi possivel alterar o Agente. Erro: " + e.getMessage());
		} finally {
			try {
				if (this.sessao.isOpen())
					sessao.close();
	...
	public void excluir(Agente agente) {
		...
		try {
			...
			System.out.println("Não foi possivel excluir o Agente. Erro: " + e.getMessage());
		} finally {
			try {
				sessao.close();
			} catch (Throwable e2) {
